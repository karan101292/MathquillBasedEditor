function MathEditor(id){
    ...
    this.default_toolbar_buttons = ["fraction","square_root","cube_root","root",'superscript','subscript','multiplication','division','plus_minus','pi','degree','not_equal','greater_equal','less_equal','greater_than','less_than','angle','parallel_to','perpendicular','triangle','parallelogram','left_bracket'];
    ...
    button_meta = {
        ...
        "left_bracket": {latex: "\\left(\\right)", tab: 1, icon:'\\left(\\right)'},    
    };
    ...
    if(true){
        setVirtualKeyboard(this.topElements,this.answerSpan,this.answerMathField);
    }
}
...
setVirtualKeyboard = function(top_elements,answer_span,field){
    ...
    jq(answer_span).focusin(function(o){
            $(this).parent().find("[id^=keys-]").slideDown();
    });
    setTimeout(function() {
        jq("[id^=keys-]").find('a').css('width',($(answer_span).width()/10)-2)
    }, 1500);
}
