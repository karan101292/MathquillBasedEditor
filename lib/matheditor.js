...
MathEditor.prototype.styleMe = function(options){
	jq(this.answerSpan).css('background',options.textarea_background).css('color',options.textarea_foreground).css('border-color',options.textarea_border).css('width',options.width).css('min-width',options.width).css('max-width',options.width).css('min-height',options.height).css('max-height',options.height).css('height',options.height);
	...
	required_tabs.forEach(function(o,idx){
		...
		required_buttons.forEach(function(b){
			if(button_meta[b].tab == idx+1){
				if(button_meta[b]){
	  				html+="<span id='matheditor-btn-span'><a data-latex='"+button_meta[b].latex+"' data-moveto='"+button_meta[b].moveto+"' data-movefor='"+button_meta[b].movefor+"' id='matheditor-btn-"+b+"' class='op-btn'><img class='matheditor-btn-img' src='lib/icons/"+button_meta[b].icon+".svg'></a></span>";
		  		}else{
